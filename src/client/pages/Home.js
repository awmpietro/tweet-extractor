...
import { Container, TabContent, TabPane, Nav, NavItem, NavLink, Card, Row, Col, Alert} from 'reactstrap';
...
export default function Home(){
    ...
    const handleSubmit = async e => {
      ...
      try {
        setLoading(true);
        const res = await axios.post(process.env.REACT_APP_API_URL, {"data" : value})
    ...
    if(!loading) {
      return (
        <Container style={{paddingTop: 50}}>
          <Row>
             <Col sm="12">
                <h1>Tweet <small> Extractor</small></h1>
              </Col>
          </Row>
          <hr/>
          <Nav tabs>
                      ...
                      <Col sm="2" className="text-right">
                        <div className="form-group">
                          <button type="submit" className="btn btn-lg btn-primary">
                            <FontAwesomeIcon color="#fff" icon={faSearch} /> Search
                          </button>
                        </div>
                      </Col>
                    </Row>
          ...
          <Alert color="danger" isOpen={isError} toggle={() => setIsError(false)}>{errorMsg}</Alert>
        </Container>
      )
    }
    return <Spin/>
